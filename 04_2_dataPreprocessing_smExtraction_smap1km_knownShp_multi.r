...
# smap pm folder
smtifPath = 'E:/GlobalSM/processedVnbasins/1km/pm'
...
for(ib in 5:8){
  ...
  write.csv(smFrac, paste(processedPath,'/','smFrac1km.csv', sep = ''), row.names = F)
  write.csv(smDat, paste(processedPath,'/','smSMAP1km.csv', sep = ''), row.names = F)
  write.csv(pointsCoord, paste(processedPath,'/','smSMAP1kmcoord.csv', sep = ''), row.names = F)
  toc()
}
