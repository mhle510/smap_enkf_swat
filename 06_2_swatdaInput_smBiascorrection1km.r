...
for(ib in 1:4){
  ...
  rawSM = read.csv(file.path(maindir, folderName,'00dataPreparation','processedSM','smSMAP1km.csv'))
  nsim = nrow(rawSM)
...
# read soil moisture from open loop simulation and plot temporal of SMAP and model--------
  ...
  opFile25 = paste(figPath,'/',basinNames[ib],'_preC_25th_hru_1km.pdf', sep = '')
  opFile50 = paste(figPath,'/',basinNames[ib],'_preC_50th_hru_1km.pdf', sep = '')
  opFile75 = paste(figPath,'/',basinNames[ib],'_preC_75th_hru_1km.pdf', sep = '')
...
# correct smap data based on open loop -------------------------------------
  ...
  opFile25 = paste(figPath,'/',basinNames[ib],'_postC_25th_hru_1km.pdf', sep = '')
  opFile50 = paste(figPath,'/',basinNames[ib],'_postC_50th_hru_1km.pdf', sep = '')
  opFile75 = paste(figPath,'/',basinNames[ib],'_postC_75th_hru_1km.pdf', sep = '')
  ...
  write.table(dqxhru, file.path(maindir, folderName, '05daens1','Project', 'dqx_hru.txt'), 
              row.names = F, col.names = F)
  write.table(rfihru, file.path(maindir, folderName, '05daens1', 'Project','rfi_hru.txt'), 
              row.names = F, col.names = F)
  
  write.table(sm4daAdj2, file.path(maindir, folderName, '05daens1', 'Project','sm_obs_mn_std_monthly.txt'), 
              row.names = F, col.names = F)
